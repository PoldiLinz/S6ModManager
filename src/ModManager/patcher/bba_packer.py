...
class BBAPacker:
    def __init__(self, user_mods_dir: str):
        ...
        system = SystemEngine()
        self.s6packer_exe = Path(system.get_bundled_s6packer_path())
        self.tools_dir = self.s6packer_exe.parent
    ...
    def prepare_pack_staging(self):
        ...
        # Maps und Texte aus dem ModLoader Staging-Verzeichnis übernehmen
        from ModManager.engines.system_engine import SystemEngine
        system = SystemEngine()
        maps_dir = Path(system.modloader_path) / "maps"
        text_dir = Path(system.modloader_path) / "text"
        
        if maps_dir.exists():
            shutil.copytree(maps_dir, self.pack_dir / "maps", dirs_exist_ok=True)
        if text_dir.exists():
            shutil.copytree(text_dir, self.pack_dir / "text", dirs_exist_ok=True)

    def prepare_mutation_staging(self, system_engine) -> bool:
        """
        Bereitet processing/pack/ für eine Mutierung vor (z.B. für Maps).
        Entpackt die aktuell aktive mod.bba nach processing/pack/.
        Gibt True zurück, wenn erfolgreich, False wenn keine aktive mod.bba gefunden wurde.
        """
        if self.pack_dir.exists():
            shutil.rmtree(self.pack_dir, ignore_errors=True)
        self.pack_dir.mkdir(parents=True, exist_ok=True)

        modloader_dir = Path(system_engine.game_path) / "modloader"
        candidate_bbas = [
            modloader_dir / "base" / "mod.bba",
            modloader_dir / "shr" / "mod.bba",
            modloader_dir / "extra1" / "mod.bba",
        ]
        
        active_bba = None
        for bba in candidate_bbas:
            if bba.exists():
                active_bba = bba
                break
                
        if not active_bba:
            # Fallback auf prepare_pack_staging
            self.prepare_pack_staging()
            return False
            
        extracted = self.unpack_archive(active_bba)
        shutil.copytree(extracted, self.pack_dir, dirs_exist_ok=True)
        self.cleanup_unpack()
        return True
